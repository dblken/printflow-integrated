...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $thickness = trim($_POST['thickness'] ?? ''); $size = trim($_POST['size'] ?? '');
    $with_stand = trim($_POST['with_stand'] ?? ''); $notes = trim($_POST['notes'] ?? '');
    if (empty($thickness) || empty($size)) {
        $error = 'Please fill in Thickness and Size.';
    } elseif (!isset($_FILES['design_file']) || $_FILES['design_file']['error'] !== UPLOAD_ERR_OK) {
        $error = 'Please upload your design.';
    } else {
        $valid = service_order_validate_file($_FILES['design_file']);
        if (!$valid['ok']) { $error = $valid['error']; } else {
            $fields = ['thickness' => $thickness ?: '3mm', 'size' => $size, 'with_stand' => $with_stand ?: 'No', 'notes' => $notes];
            $result = service_order_create('Stickers on Sintraboard', $customer_id, $fields, [['file' => $_FILES['design_file'], 'prefix' => 'design']]);
            if ($result['success']) { $_SESSION['order_success_id'] = $result['order_id']; redirect(BASE_URL . '/customer/order_success.php?service=sintraboard'); }
...
<div class="min-h-screen py-8">
    <div class="container mx-auto px-4" style="max-width: 640px;">
        <h1 class="text-2xl font-bold text-gray-900 mb-6">Stickers on Sintraboard</h1>
            ...
            <form method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-4">
                    ...
                    <select name="thickness" class="input-field" required>
                        <option value="3mm">3mm</option><option value="5mm">5mm</option>
                    </select>
                </div>
                <div class="mb-4">
                    <label class="block text-sm font-medium text-gray-700 mb-1">Size *</label>
                    <input type="text" name="size" class="input-field" required placeholder="e.g. 18x24 inches" value="<?php echo htmlspecialchars($_POST['size'] ?? ''); ?>">
                </div>
                <div class="mb-4">
