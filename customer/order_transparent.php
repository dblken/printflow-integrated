...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $size = trim($_POST['size'] ?? ''); $with_white_ink = trim($_POST['with_white_ink'] ?? '');
    ...
    if (empty($size) || $quantity < 1) {
        ...
        if (!$valid['ok']) { $error = $valid['error']; } else {
            $fields = ['size' => $size, 'with_white_ink' => $with_white_ink ?: 'No', 'quantity' => $quantity, 'notes' => $notes];
            $result = service_order_create('Transparent Stickers', $customer_id, $fields, [['file' => $_FILES['design_file'], 'prefix' => 'design']]);
            if ($result['success']) { $_SESSION['order_success_id'] = $result['order_id']; redirect(BASE_URL . '/customer/order_success.php?service=transparent'); }
...
<div class="min-h-screen py-8">
    <div class="container mx-auto px-4" style="max-width: 640px;">
        <h1 class="text-2xl font-bold text-gray-900 mb-6">Transparent Stickers</h1>
        ...
        <div class="card">
            <form method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="size" class="input-field" required placeholder="e.g. 4x6 inches" value="<?php echo htmlspecialchars($_POST['size'] ?? ''); ?>">
                </div>
                <div class="mb-4">
                    <label class="block text-sm font-medium text-gray-700 mb-1">With White Ink?</label>
                    <select name="with_white_ink" class="input-field">
