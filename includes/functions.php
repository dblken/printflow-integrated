...
function get_unread_notification_count($user_id, $role) {
    $field = ($role === 'Customer') ? 'customer_id' : 'user_id';
    
    $sql = "SELECT COUNT(*) as count FROM notifications WHERE {$field} = ? AND is_read = 0";
 ...
 * @param array $params Optional query parameters to preserve
 * @return string HTML
 */
function get_pagination_links($current_page, $total_pages, $params = []) {
    ...
    $links = '<div class="flex items-center justify-center gap-2 mt-8">';
    
    // Remove 'page' from params to avoid duplication
    unset($params['page']);
    $query = count($params) > 0 ? http_build_query($params) . '&' : '';
    
    // First & Previous
    if ($current_page > 1) {
        $links .= "<a href='{$url}?{$query}page=1' class='pagination-btn' title='First'>&laquo;</a>";
        $prev = $current_page - 1;
        $links .= "<a href='{$url}?{$query}page={$prev}' class='pagination-btn'>&lsaquo;</a>";
    }
    ...
    for ($i = $start; $i <= $end; $i++) {
        $activeClass = $i == $current_page ? 'pagination-btn-active' : 'pagination-btn';
        $links .= "<a href='{$url}?{$query}page={$i}' class='{$activeClass}'>{$i}</a>";
    }
    
    // Next & Last
    if ($current_page < $total_pages) {
        $next = $current_page + 1;
        $links .= "<a href='{$url}?{$query}page={$next}' class='pagination-btn'>&rsaquo;</a>";
        $links .= "<a href='{$url}?{$query}page={$total_pages}' class='pagination-btn' title='Last'>&raquo;</a>";
    }
